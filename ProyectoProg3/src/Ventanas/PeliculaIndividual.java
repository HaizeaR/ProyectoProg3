...
/**
 * Ventana donde se muestra información de la perlicula individual con sus
 * horarios
 * 
...
public class PeliculaIndividual extends JFrame {
	...
	public static void conseguirIdSala(String codS) {
		...
		try {
			...
			sentSQL = "select * from sesion where cod_sesion = '" + codS + "'";
			System.out.println(sentSQL);
			...
			while (rs.next()) {
				...
				codSala = rs.getString("cod_sesion");
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	private void mostrar() {
		...
		try {
			...
			while (rs.next()) {
				...
				modelo.addRow(
						new Object[] { rs.getString("cod_sesion"), rs.getString("fecha"), rs.getString("horaI") });
	...
	// Un HashMap que tenga como clave un String ( nombre de peli) y un SET que
	// tenga
